  ...
  return (
    <nav className={`bottom-nav ${collapsed ? 'collapsed' : ''}`}>
      <div className="nav-logo desktop-only">
        <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: '8px', textDecoration: 'none', color: '#14b8a6', fontWeight: '800', fontSize: collapsed ? '0' : '1.4rem' }}>
          <img src={logoUrl} alt="Que-Sale Logo" style={{ width: collapsed ? '36px' : '40px', height: collapsed ? '36px' : '40px', borderRadius: '10px' }} />
      ...
      <Link to="/contacto" className={location.pathname === '/contacto' ? 'nav-item active' : 'nav-item'}>
        <BellIcon size={24} />
        <span className="nav-label desktop-only">Notificaciones</span>
      </Link>
